...
workflow GATKSVPipelineBatch {
  ...
  scatter (i in range(length(samples))) {
    File counts_files_index_ = counts_files_[i] + ".tbi"
  ...
  if (defined(GATKSVPipelinePhase1.std_manta_vcf)) {
    scatter (i in range(length(samples))) {
      File std_manta_vcfs_index_ = select_first([GATKSVPipelinePhase1.std_manta_vcf])[i] + ".tbi"
    }
  }
  if (defined(GATKSVPipelinePhase1.std_melt_vcf)) {
    scatter (i in range(length(samples))) {
      File std_melt_vcfs_index_ = select_first([GATKSVPipelinePhase1.std_melt_vcf])[i] + ".tbi"
    }
  }
  if (defined(GATKSVPipelinePhase1.std_wham_vcf)) {
    scatter (i in range(length(samples))) {
      File std_wham_vcfs_index_ = select_first([GATKSVPipelinePhase1.std_wham_vcf])[i] + ".tbi"
    }
  }
  ...
  output {
    ...
    Array[File] counts_index = counts_files_index_
    ...
    Array[File]? std_manta_vcfs_index = std_manta_vcfs_index_
    Array[File]? std_melt_vcfs = GATKSVPipelinePhase1.std_melt_vcf
    Array[File]? std_melt_vcfs_index = std_melt_vcfs_index_
    Array[File]? std_wham_vcfs = GATKSVPipelinePhase1.std_wham_vcf
    Array[File]? std_wham_vcfs_index = std_wham_vcfs_index_
