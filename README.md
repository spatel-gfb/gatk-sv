...
> cp $GATK_SV_ROOT/inputs/build/GATKSVPipelineBatch.ref_panel_1kg.json GATKSVPipelineBatch.my_run.json
...
`GATKSVPipelineSingleSample.wdl` runs the pipeline on a single sample using a fixed reference panel. An example run with reference panel containing 156 samples from the [NYGC 1000G Terra workspace](https://app.terra.bio/#workspaces/anvil-datastorage/1000G-high-coverage-2019) can be found at `inputs/build/GATKSVPipelineSingleSample.ref_panel_1kg.na12878.json` after [building inputs](#Building inputs)). 
